...
- 최대 너비 `max-w-2xl` (672px) 중앙 정렬 기본 레이아웃
- 터치 타겟 최소 44px (`py-2 px-3` 이상)
- 하단 네비게이션 고려하여 `pb-20` 여백 확보
- 텍스트 크기: 모바일에서 읽기 편한 `text-sm` ~ `text-base` 기본

## 관리자 UI (데스크톱 전용)
- 관리자 (`/admin/*`) 페이지는 **데스크톱 전용** — 모바일 반응형 불필요
- 레이아웃: 고정 좌측 사이드바 (`w-56 bg-navy`) + 우측 콘텐츠 영역 (`flex-1`)
- 사이드바: 세로 네비게이션 (아이콘 + 텍스트), 하단에 "365 성경읽기" 복귀 링크 + 로그아웃
- 각 페이지: `h2 text-xl font-bold` 타이틀 + accent bar → 콘텐츠
- 콘텐츠 영역: `max-w` 제한 없음, 전체 너비 사용 가능
- ERP/CRM 스타일: 분할 패널, 테이블, 필터 등 정보 밀도 높은 UI 허용
