...
// 주보 생성
export async function createJuboPost(
  formData: FormData,
  ...
  if (validFiles.length > 0) {
    ...
    if (imageRecords.length > 0) {
      await admin.from("jubo_images").insert(imageRecords);
    }
  }
...
// 주보 수정
export async function updateJuboPost(
  formData: FormData,
): Promise<{ id?: number; error?: string }> {
  const auth = await checkAdmin();
  if ("error" in auth && !("admin" in auth)) return { error: auth.error };
  ...
  if (removedImagesRaw) {
    ...
    for (const fileName of removedImages) {
      await deleteFromR2(`jubo/${fileName}`);
    }
    await admin
      .from("jubo_images")
      .delete()
  ...
  revalidatePath("/jubo");
  revalidatePath(`/jubo/${postId}`);
  return { id: postId };
}

// 주보 삭제
export async function deleteJuboPost(
  postId: number,
  ...
  const [{ data: post }, { data: images }] = await Promise.all([
    admin.from("jubo_posts").select("content").eq("id", postId).single(),
    admin.from("jubo_images").select("file_name").eq("post_id", postId),
  ]);
  ...
  if (images && images.length > 0) {
    await Promise.all(
      images.map((img) => deleteFromR2(`jubo/${img.file_name}`)),
    );
  }

  // 게시글 삭제 (CASCADE로 jubo_images도 삭제됨)
