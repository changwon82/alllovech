    ...
    <>
      ...
      {q && (
        <div className="mt-4 flex items-center gap-2 text-sm text-neutral-500">
          ...
          <a href="/news" className="rounded-lg px-2 py-1 text-xs font-medium text-navy transition hover:bg-navy/10">초기화</a>
        </div>
      )}
          ...
          <tbody>
            {posts.map((post) => (
              <tr key={post.id} className="border-b border-neutral-100">
                ...
                <td className="py-2.5">
                  <Link href={q ? `/news/${post.id}?q=${encodeURIComponent(q)}` : `/news/${post.id}`} className="line-clamp-1 font-semibold text-neutral-800 transition hover:text-navy md:line-clamp-none">
      ...
      {totalPages > 1 && (() => {
        ...
        const baseHref = (p: number) => { sp.set("page", String(p)); return `/news?${sp.toString()}`; };
        const pages: number[] = [];
      ...
      <BoardSearch table="news_posts" basePath="/news" defaultValue={q} defaultField={sf as any} />
    </>
  );
}
