...
export default async function BrothersPage({
  ...
  return (
    <>
          ...
          <tbody>
            {posts.map((post) => (
              <tr key={post.id} className="border-b border-neutral-100">
                ...
                <td className="py-2.5">
                  <Link href={`/brothers/${post.id}`} className="line-clamp-1 font-semibold text-neutral-800 transition hover:text-navy md:line-clamp-none">
                    {post.title}
                  </Link>
                </td>
              </tr>
      ...
      {/* 페이지네이션 */}
      {totalPages > 1 && (() => {
        const baseHref = (p: number) => `/brothers?page=${p}`;
        const pages: number[] = [];
