        ...
        <div className="space-y-2">
          {months.map(({ month, days }) => {
            ...
            <div key={month} className="flex items-start gap-1">
              <div className="flex shrink-0 items-baseline" style={{ width: 72 }}>
                <span className="text-xs font-medium text-neutral-500" style={{ minWidth: 28, textAlign: "right" }}>{getMonthLabel(month)}</span>
                <span className="ml-1 text-[10px] tabular-nums text-neutral-400" style={{ minWidth: 36, textAlign: "right" }}>{checked}/{past}</span>
              </div>
